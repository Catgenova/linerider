...
namespace CyberRider.Unity
{
    ...
    public sealed class Hud
    {
        ...
        public Hud(Transform canvas, GameController game, System.Action onMenu)
        {
            ...
            UiKit.Place(_topLeft, 0, 1, 12, -10, compact ? 300 : 420, 44, 0, 1);
            ...
            // Top centre: ink bars.
            float inkW = compact ? 200 : 260;
            RectTransform center = UiKit.Rect("TopCenter", Root);
            UiKit.Place(center, 0.5f, 1, 0, -8, inkW + 20, 52, 0.5f, 1);
            _inkFillImage = BuildInkBar(center, 0, inkW, out _inkFill, out _inkText, UiKit.Cyan);
            _inkBox2 = UiKit.Rect("InkBox2", center);
            UiKit.Place(_inkBox2, 0.5f, 1, 0, -26, inkW + 20, 24, 0.5f, 1);
            BuildInkBar(_inkBox2, 0, inkW, out _inkFill2, out _inkText2, UiKit.Magenta);

            // Top right: stats.
            _topRight = UiKit.Rect("TopRight", Root);
            UiKit.Place(_topRight, 1, 1, -12, -8, compact ? 260 : 520, 44, 1, 1);
            _stats = UiKit.Label(_topRight, "", 13, Color.white, TextAnchor.MiddleRight, true);
            UiKit.Stretch(_stats.rectTransform);

            // Objectives: under the title, or beside the toolbar on phones.
            _objectives = UiKit.Rect("Objectives", Root);
            if (compact) UiKit.Place(_objectives, 0, 1, 176, -64, 300, 160, 0, 1);
            else UiKit.Place(_objectives, 0, 1, 12, -62, 320, 200, 0, 1);
            var ov = UiKit.VBox(_objectives, "List", 4);
            UiKit.Stretch(ov.GetComponent<RectTransform>());
            ov.childForceExpandHeight = false;

            // Toolbar (left middle): a column, or a two-column grid of finger-sized buttons on phones.
            _toolbar = UiKit.Rect("Toolbar", Root);
            if (compact)
            {
                UiKit.Place(_toolbar, 0, 0.5f, 12, 40, 152, 240, 0, 0.5f);
                var tg = UiKit.Grid(_toolbar, "List", 74, 36, 4);
                var tgrt = tg.GetComponent<RectTransform>();
                tgrt.anchorMin = new Vector2(0, 1);
                tgrt.anchorMax = new Vector2(1, 1);
                tgrt.pivot = new Vector2(0.5f, 1);
            ...
            // Material palette (bottom left) and object palette. Phones run the materials along the
            // bottom up to the playback bar and hang the object list under the top bar instead.
            _objectPalette = UiKit.Rect("ObjectPalette", Root);
            _palette = UiKit.Rect("Palette", Root);
            if (compact)
            {
                _palette.anchorMin = new Vector2(0, 0);
                _palette.anchorMax = new Vector2(1, 0);
                _palette.pivot = new Vector2(0, 0);
                _palette.offsetMin = new Vector2(12, 12);
                _palette.offsetMax = new Vector2(-460, 96);
                _objectPalette.anchorMin = new Vector2(0, 1);
                _objectPalette.anchorMax = new Vector2(1, 1);
                _objectPalette.pivot = new Vector2(0, 1);
                _objectPalette.offsetMin = new Vector2(176, -184);
                _objectPalette.offsetMax = new Vector2(-12, -64);
            }
            else
            {
            ...
            // Playbar: bottom centre, or bottom right on phones.
            _playbar = UiKit.Rect("Playbar", Root);
            if (compact) UiKit.Place(_playbar, 1, 0, -12, 12, 420, 40, 1, 0);
            else UiKit.Place(_playbar, 0.5f, 0, 0, 12, 560, 40, 0.5f, 0);
            ...
            if (compact) _hint.gameObject.SetActive(false);
            Rebuild();
        }

        private static Image BuildInkBar(Transform parent, float yOffset, float width, out RectTransform fill, out Text text, Color color)
        {
            RectTransform box = UiKit.Rect("InkBox", parent);
            UiKit.Place(box, 0.5f, 1, 0, yOffset, width, 24, 0.5f, 1);
            Image bar = UiKit.Panel(box, "Bar", new Color(0, 0, 0, 0.5f));
            UiKit.Place(bar.rectTransform, 0.5f, 1, 0, 0, width, 8, 0.5f, 1);
            ...
            UiKit.Place(text.rectTransform, 0.5f, 1, 0, -10, width, 16, 0.5f, 1);
            return fillImg;
        }
        ...
        public void Rebuild()
        {
            ...
            var tools = new[]
            {
                (ToolId.Pencil, "Pencil (P)", "Pencil"), (ToolId.Line, "Line (L)", "Line"), (ToolId.Eraser, "Eraser (E)", "Eraser"), (ToolId.Flip, "Flip (V)", "Flip"), (ToolId.Pan, "Pan (H)", "Pan"), (ToolId.Object, "Objects (O)", "Objects"),
            };
            foreach (var (id, label, shortLabel) in tools)
            {
                if (id == ToolId.Object && !editor.Constraints.CanPlaceObjects) continue;
                ToolId captured = id;
                UiKit.Button(tv, compact ? shortLabel : label, () => game.SetTool(captured), editor.Tool == id ? UiKit.Cyan : (Color?)UiKit.Muted, 11, 26);
            }
            if (!compact) UiKit.Spacer(tv, 4);
            UiKit.Button(tv, compact ? "Undo" : "Undo (Ctrl+Z)", () => editor.Undo(), UiKit.Muted, 11, 26);
            UiKit.Button(tv, compact ? "Redo" : "Redo (Ctrl+Y)", () => editor.Redo(), UiKit.Muted, 11, 26);
            ...
            {
                if (!compact) UiKit.Spacer(tv, 4);
                UiKit.Button(tv, compact ? "Start" : "Set start", () => game.BeginPlacement("start"), UiKit.Muted, 11, 26);
                UiKit.Button(tv, compact ? "Finish" : "Set finish", () => game.BeginPlacement("finish"), UiKit.Muted, 11, 26);
            }
            if (game.CoopActive)
            {
                if (!compact) UiKit.Spacer(tv, 4);
                string other = "P" + (game.CoopPlayer == 1 ? 2 : 1);
                UiKit.Button(tv, compact ? "To " + other : "Switch to " + other + " (Tab)", () => game.SwitchCoopPlayer(), game.CoopPlayer == 1 ? UiKit.Cyan : UiKit.Magenta, 11, 26);
            }

            UiKit.Clear(_objectPalette);
            if (editor.Constraints.CanPlaceObjects && editor.Tool == ToolId.Object)
            {
                var grid = UiKit.Grid(_objectPalette, "Grid", compact ? 96 : 108, compact ? 34 : 24, 4);
            ...
            var pg = UiKit.Grid(_palette, "Grid", compact ? 64 : 90, compact ? 34 : 24, 4);
            var prt = pg.GetComponent<RectTransform>();
            prt.anchorMin = new Vector2(0, 0);
            prt.anchorMax = new Vector2(1, 0);
            prt.pivot = new Vector2(0, 0);
            prt.anchoredPosition = Vector2.zero;
            if (compact) prt.sizeDelta = Vector2.zero;
            foreach (MaterialId id in editor.Constraints.Materials)
            {
                Core.Material m = Materials.Get(id);
                MaterialId captured = id;
                UiKit.Button(pg.transform, compact ? m.Name : m.Name + " " + m.Hotkey, () => game.SetMaterial(captured), U.Hex(m.Color, editor.Material == id ? 1f : 0.45f), 10, 24);
            }

            UiKit.Clear(_playbar);
            var pb = UiKit.HBox(_playbar, "Row", 4, 0, TextAnchor.MiddleCenter);
            var pbrt = pb.GetComponent<RectTransform>();
            float ax = compact ? 1f : 0.5f;
            pbrt.anchorMin = new Vector2(ax, 0);
            pbrt.anchorMax = new Vector2(ax, 0);
            pbrt.pivot = new Vector2(ax, 0);
            ...
            int pf = compact ? 12 : 14;
            UiKit.Button(pb.transform, playing ? "PAUSE" : "PLAY", () => game.TogglePlay(), UiKit.Lime, pf, 32, 44);
            UiKit.Button(pb.transform, "STOP", () => game.Stop(), null, pf, 32, 34);
            UiKit.Button(pb.transform, compact ? "RESET" : "RESTART", () => game.Restart(true), null, pf, 32, 34);
            ...
            double[] speeds = compact ? new[] { 1.0, 2.0 } : new[] { 0.5, 1, 2, 4 };
            foreach (double s in speeds)
            {
                double captured = s;
                UiKit.Button(pb.transform, U.F(s, "0.#") + "x", () => game.SetSpeed(captured), game.Speed == s ? UiKit.Cyan : (Color?)UiKit.Muted, 11, 32, compact ? 36 : 40);
            }
            UiKit.Button(pb.transform, "ghost", () => game.ToggleGhost(), game.GhostEnabled ? UiKit.Cyan : (Color?)UiKit.Muted, 11, 32, compact ? 40 : 50);
            UiKit.Button(pb.transform, compact ? "SND" : "SOUND", () => game.ToggleMusic(), game.Audio != null && game.Audio.Enabled ? UiKit.Cyan : (Color?)UiKit.Muted, 12, 32, 34);
            ...
            _hint.text = game.Mode == GameMode.Arcade
                ...
                : InputBridge.TouchSupported
                    ? "One finger draws - two fingers pan and zoom - Flip tool taps a line"
